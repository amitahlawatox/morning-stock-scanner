...
from .symbols import fetch_sp500_tickers
...
def run_pipeline(*, dry_run: bool = False):
    ...
    print("\n[2/7] Fetching general market news …")
    general_news = fetch_general_news()
    print(f"       → {len(general_news)} macro headlines")

    print(f"\n[2/7] Scanning company news for {len(tickers)} tickers …")
    ...
    print("\n[3/7] Stage 1: Sentiment Filter (→ top 15) …")
    ...
    print("\n[4/7] Stage 2: Geopolitical Cross-Reference (→ top 10) …")
    ...
    # ── Step 5: Stage 3 – Technical Filter ──────────────────
    print(f"\n[5/7] Stage 3: Pre-Market Technical Filter (→ top 5) …")
    print(f"       (estimated time: ~{len(stage2) * 12 / 60:.0f} minutes at 12s/ticker)")
    stage3 = technical_filter(stage2)
    ...
    # ── Step 6: Stage 4 – Stress-Test ───────────────────────
    print("\n[6/7] Stage 4: Devil's Advocate Stress-Test (→ top 2) …")
